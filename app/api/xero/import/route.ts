  ...
  try {
    ...
    async function fetchWithRefreshAndRateLimit(apiUrl: string) {
      ...
      while (attempt <= XERO_MAX_RETRIES) {
        ...
        await sleep(XERO_RATE_LIMIT_WAIT_MS);
        attempt += 1;
      }
    ...
    const revenueAccounts = accounts
      ...
      .map((account) => {
        ...
        if (code) revenueAccountCodes.push(code);
    ...
    function isRevenueAccount(accountCode: string | undefined) {
      ...
      return isXeroRevenueAccount(accountMap.get(code));
    }
    ...
    for (const summary of batch) {
      ...
      if (source === "invoices") {
        ...
        for (const [lineIndex, line] of (invoice.LineItems || []).entries()) {
          ...
          addImportedLineRow({ recordId, lineIndex, recordDate, line });
    ...
    for (const row of allImportedLines || []) {
      const bucket = recalculatedBuckets.find((item) => item.month_label === row.month_label);
      if (!bucket) continue;
    ...
    return NextResponse.json({
      message: "Production Xero batch import complete",
      clientId,
    ...
    return NextResponse.json(
      {
        error: "Unexpected production Xero batch import failure",
