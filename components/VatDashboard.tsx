...
import {
  ...
  VAT_DEREGISTRATION_THRESHOLD,
  MonthlyTurnover,
  currency,
  getVatRisk,
  isForwardLookTriggered,
  riskClasses,
  rollingTaxableTurnover,
  taxableTurnover,
} from "@/lib/vat";

const demoRows: MonthlyTurnover[] = [
...
export default function VatDashboard() {
  ...
  const [rows, setRows] = useState<MonthlyTurnover[]>(demoRows);

  const last12Rows = useMemo(() => rows.slice(-12), [rows]);
  const rollingTotal = useMemo(() => rollingTaxableTurnover(rows), [rows]);
  const exemptTotal = useMemo(() => last12Rows.reduce((sum, row) => sum + Number(row.exempt || 0), 0), [last12Rows]);
  const outOfScopeTotal = useMemo(() => last12Rows.reduce((sum, row) => sum + Number(row.outOfScope || 0), 0), [last12Rows]);
  const risk = getVatRisk(rollingTotal);
  const forwardTriggered = isForwardLookTriggered(expected30Days);
  const thresholdRemaining = Math.max(VAT_REGISTRATION_THRESHOLD - rollingTotal, 0);
  const percentageUsed = Math.min((rollingTotal / VAT_REGISTRATION_THRESHOLD) * 100, 100);
  ...
  function exportReviewNote() {
    const content = [
      ...
      `VAT registration threshold: ${currency(VAT_REGISTRATION_THRESHOLD)}`,
      ...
      `Forward-looking trigger: ${forwardTriggered ? "Yes" : "No"}`,
      ...
      <div className="mx-auto max-w-7xl p-4 md:p-8">
        <header className="mb-6 rounded-3xl bg-slate-950 p-6 text-white shadow-xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-slate-300">Maddock & Co. UK Ltd</p>
              <h1 className="mt-2 text-3xl font-bold md:text-4xl">VAT Registration Checker</h1>
              <p className="mt-2 max-w-2xl text-slate-300">UK VAT threshold monitoring dashboard for accountant-led client reviews.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4 text-sm">
              <div>Registration threshold</div>
              <div className="text-2xl font-semibold">{currency(VAT_REGISTRATION_THRESHOLD)}</div>
              <div className="mt-1 text-slate-300">Deregistration: {currency(VAT_DEREGISTRATION_THRESHOLD)}</div>
            </div>
          </div>
        </header>
        ...
        <section className="grid gap-4 lg:grid-cols-4">
          ...
          <div className={`rounded-3xl border-2 p-5 shadow-sm lg:col-span-3 ${riskClasses(risk.status)}`}>
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <div className="grid gap-3 text-sm md:grid-cols-2">
                ...
                <Metric label="Threshold used" value={`${((rollingTotal / VAT_REGISTRATION_THRESHOLD) * 100).toFixed(1)}%`} />
            ...
            <div className="mt-6 h-4 overflow-hidden rounded-full bg-white/70">
              <div className="h-full rounded-full bg-slate-950" style={{ width: `${percentageUsed}%` }} />
            </div>
        ...
        <section className="mt-4 grid gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border bg-white p-5 shadow-sm lg:col-span-2">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Manual input is live. Database save and CSV import are prepared in the project structure.</p>
              </div>
              <button onClick={addMonth} className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white">+ Add month</button>
            </div>
                ...
                <tbody>
                  {rows.map((row, index) => (
                    <tr key={`${row.month}-${index}`} className="border-t bg-white">
                      {(["month", "standardRated", "reducedRated", "zeroRated", "exempt", "outOfScope"] as const).map((field) => (
                      ...
                      <td className="p-2">
                        <button type="button" onClick={() => removeMonth(index)} className="rounded-lg px-3 py-2 text-slate-500 hover:bg-slate-100">×</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="rounded-3xl border bg-white p-5 shadow-sm">
            ...
            <button onClick={exportReviewNote} className="mt-4 w-full rounded-xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white">
              Export review note
            </button>
          </div>
        </section>
